...
class ImpalaCNNLarge(nn.Module):
    ...
    """
    def __init__(self, in_depth, actions, model_size=4, spectral_norm='all'):
        super().__init__()
    ...
    def reset_mlps(self):
        self.dueling = Dueling(
            nn.Sequential(nn.Linear(2048*self.model_size, 256),
                          nn.ReLU(),
                          nn.Linear(256, 1)),
            nn.Sequential(nn.Linear(2048*self.model_size, 256),
                          nn.ReLU(),
    ...
    def load_checkpoint(self):
        ...
        self.load_state_dict(torch.load("current_model260538"))
