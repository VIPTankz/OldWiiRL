...
class ProcessFrameUint(gym.ObservationWrapper):
    def __init__(self, env=None):
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(60, 112, 1), dtype=np.uint8)
    ...
    def process(im):
        ...
        im = np.reshape(im, [60, 112, 1])#.astype(np.float32)
...
def wrap_env_vec(env, stack_frames=4):
