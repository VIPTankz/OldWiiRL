...
class MarioKartEnv():
    ...
    def reset(self):
        ...
        if ran < 0.2:
            ...
            KeyPress("n")
            self.current_chkp = 34
            self.chkp_count = 35

        elif ran < 0.6:
            KeyPress("l")
            self.current_chkp = 28
            self.chkp_count = 29

        elif ran < 0.8:
            KeyPress("k")
            self.current_chkp = 46
            self.chkp_count = 47
        else:
            KeyPress("j")
            self.current_chkp = 52
            self.chkp_count = 53
        ...
        self.dist_x = np.array([10,10,10,10,10,10,10,10,10,10,10],dtype=np.float32)
        self.dist_y = np.array([10,10,10,10,10,10,10,10,10,10,10],dtype=np.float32)

        self.focal_dists = np.array([10,10,10,10,10,10,10,10,10,10,10],dtype=np.float32)
        
        self.test_timer = time.time()
        self.rewards_claimed = [False,False,False,False,False,False]
        self.held_reward = 0
    ...
    def template_match(self,img):
        ...
        #cv2.imwrite("map_region_rainbow_road.jpg", img)
        ...
        if not self.first:
            ...
            new_focal_dist = old_dist_to_focal - new_dist_to_focal
            self.focal_dists = np.roll(self.focal_dists,1)
            self.focal_dists[0] = new_focal_dist
            
            reward = new_focal_dist / 20
            if reward < 0:
                reward = 0
            elif reward > .15:
                reward = 0.15
        ...
        self.dist_x = np.roll(self.dist_x,1)
        self.dist_x[0] = x_dif
        
        self.dist_y = np.roll(self.dist_y,1)
        self.dist_y[0] = y_dif
    ...
    def get_reward(self,x_dif,y_dif):
        ...
        """if self.regions[num].is_chkp:
            print("chkp")
            print(self.regions[num].chkp_num)"""
        

        if self.regions[num].is_chkp:
            if self.regions[num].chkp_num == self.current_chkp - 1 or self.regions[num].chkp_num == self.current_chkp - 2:
                self.out_frames = 12
                #print("Death by backward")
                #print("check: " + str(self.regions[num].chkp_num))
                return 0

        #checkpoints
        if self.regions[num].is_chkp:
            if not(self.regions[num].chkp_num > 5 and (self.current_chkp == -1 or self.current_chkp == 0)):
                if self.regions[num].chkp_num == self.current_chkp + 1 or self.regions[num].chkp_num == self.current_chkp + 2 or \
                   (self.regions[num].chkp_num == 0 and self.current_chkp == self.num_chkps):
                    ...
                    print("Checkpoint: " + str(self.current_chkp))
        ...
        """if self.chkp_count > 0:
            ...
            if (np.mean(self.avg_time) + (time.time() - self.checkpoint_timer)) / 4 > self.time_till_checkpoint:
                self.out_frames = 10"""
                #print("Death by checkpoint timer")
        ...
        """if np.mean(self.avg_dist) < 0.15 and self.c_frames > 30:
            #print("Death by slowness")
            self.out_frames = 10"""

        dist = self.compute_dist()
        #0.08
        
        if self.c_frames < 103 and self.vul:
            mult = 3.2
        else:
            mult = 1

        if dist < 0.05 * mult and self.c_frames > 30:
            self.out_frames = 10
            #print("Death by Dist")
            #kill
    ...
    def compute_dist(self):
        ...
        x = np.mean(self.dist_x)
        y = np.mean(self.dist_y)
    ...
    def step(self,action=0):
        ...
        if self.drift_held > 22 and not self.rewards_claimed[0]:
            reward += 0.1
            self.rewards_claimed[0] = True

        elif self.drift_held > 26 and not self.rewards_claimed[1]:
            reward += 0.2
        ...
        elif self.drift_held > 34 and not self.rewards_claimed[3]:
            reward += 0.35
            self.rewards_claimed[3] = True

        elif self.drift_held > 38 and not self.rewards_claimed[4]:
            reward += 0.45
            self.rewards_claimed[4] = True
            
        elif self.drift_held > 42 and not self.rewards_claimed[5]:
            reward += 0.75
    ...
    def end_drift(self,bonus=False):
        ...
        if self.drift_held > 40:
            self.held_reward = 0.75
            #if bonus:
                #self.held_reward = 1.5
    ...
    def apply_action(self,action):
        ...
        #soft drifts
            ...
            if self.direction == 1:
                ...
                self.held_keys = ["w","c","d"]
                self.drift_held += 1.05
                if self.act_timer >= self.act_timer_max:
                    self.direction = 1
                self.act_timer += 1                
            else:
                self.held_keys = ["w"]
